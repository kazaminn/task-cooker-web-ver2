...
const authHost = process.env.FIREBASE_AUTH_EMULATOR_HOST || process.env.AUTH_EMULATOR_HOST;
...
async function seed() {
  ...
  try {
    // 1. User Creation
    const usersData = [
      { uid: 'user-alice', email: 'alice@example.com', name: 'アリス', seed: 'Alice' },
      { uid: 'user-bob', email: 'bob@example.com', name: 'ボブ', seed: 'Bob' },
      { uid: 'user-charlie', email: 'charlie@example.com', name: 'チャーリー', seed: 'Charlie' },
      { uid: 'user-dave', email: 'dave@example.com', name: 'デイブ', seed: 'Felix' },
    ...
    const projectsData = [
      { 
        ...
        taskCandidates: [
          { title: '材料の買い出し', desc: '薄力粉、卵、牛乳、メープルシロップを購入する' },
          { title: '生地の作成', desc: '卵と牛乳を混ぜた後、粉をふるいながら入れる' },
          { title: 'トッピングの準備', desc: 'フルーツをカットし、ホイップクリームを作る' },
      ...
      { 
        ...
        taskCandidates: [
          ...
          { title: '当日のタイムスケジュール作成', desc: '集合から片付けまでの流れを固める' }
        ],
        mixTitle: '雨天時の対応',
      ...
      { 
        ...
        overview: '# 開発ロードマップ\n3月末のリリースを目指します。\n\n- React + Tailwind 4.0\n- Firebase Backend',
        taskCandidates: [
          { title: '認証機能の実装', desc: 'Googleログインとメール認証を有効にする' },
          { title: 'Firestoreスキーマ設計', desc: 'チームとプロジェクトの階層構造を定義' },
          { title: 'Storybookのセットアップ', desc: '主要なUIコンポーネントをカタログ化する' },
          { title: 'タスク一覧画面の構築', desc: 'フィルタリング機能を含むUIを実装' },
      ...
      { 
        ...
        overview: '# My Life\n個人的な目標と日々のタスク管理。',
        taskCandidates: [
          { title: '読書（技術書）', desc: '今月はReactのデザインパターンを読み切る' },
        ...
        mixTitle: '今月の振り返り',
        mixPost: '今週は少しタスクを詰め込みすぎたかもしれない。来週は余裕を持たせよう。'
      },
    ];
    ...
    for (const p of projectsData) {
      ...
      const mix: Mix = {
        ...
        projectRef: p.id,
        authorId: author.id!,
        author: { id: author.id!, name: author.displayName, photoURL: author.photoURL },
        title: p.mixTitle,
        status: 'open',
        isPublic: true,
    ...
    console.log(`✅ ${projectsData.length} projects set up with realistic data.`);
